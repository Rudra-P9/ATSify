...
import { parseJobDescription } from '../../lib/parser/job-parser';
...
export default function JDInput() {
    ...
    const renderHighlightedText = () => {
        ...
        const skills = new Set(parsed.extractedSkills.map(s => s.toLowerCase()));
        const lines = jobDescription.split('\n');

        return lines.map((line, lineIdx) => {
            if (!line.trim()) return <p key={lineIdx} className="h-4" />;

            // Simple word-by-word highlighting (could be improved with n-grams)
            const words = line.split(/(\s+|[,.;:()])/);
            
            return (
                <p key={lineIdx} className="mb-2 text-white/60 leading-relaxed">
                    {words.map((part, partIdx) => {
                        const normalized = part.toLowerCase();
                        const isSkill = skills.has(normalized);
                        
                        if (isSkill) {
                            const isRequired = parsed.requiredSkills.some(s => s.toLowerCase() === normalized);
                            const isPreferred = parsed.preferredSkills.some(s => s.toLowerCase() === normalized);
                            
                            return (
                                <span 
                                    key={partIdx}
                                    onMouseEnter={() => setHoveredKeyword(normalized)}
                                    onMouseLeave={() => setHoveredKeyword(null)}
                                    className={cn(
                                        "px-1 rounded cursor-help transition-all relative group/kw",
                                        isRequired 
                                            ? "bg-indigo-500/20 text-indigo-300 font-bold border-b border-indigo-500/40" 
                                            : "bg-emerald-500/10 text-emerald-300 border-b border-emerald-500/20"
                                    )}
                                >
                                    {part}
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 hidden group-hover/kw:block z-50">
                                        <div className="bg-[#1a1a1f] border border-white/10 rounded-lg p-2 shadow-2xl scale-95 origin-bottom animate-in zoom-in-95 duration-200">
                                            <div className="flex items-center gap-1.5 mb-1">
                                                {isRequired ? <CheckCircle2 className="w-3 h-3 text-indigo-400" /> : <Star className="w-3 h-3 text-emerald-400" />}
                                                <span className="text-[10px] font-black uppercase tracking-widest text-white">
                                                    {isRequired ? 'Core Requirement' : 'Preferred Skill'}
                                                </span>
                                            </div>
                                            <p className="text-[9px] text-white/40 leading-tight">
                                                High impact keyword for ATS ranking in this {parsed.industryContext} role.
                                            </p>
                                        </div>
                                    </div>
                                </span>
