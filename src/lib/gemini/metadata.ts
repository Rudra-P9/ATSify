...
function countPositions(text: string): number {
  ...
  const patterns: RegExp[] = [
    /\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(19|20)\d{2}\s*[-–—]\s*(present|current|(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(19|20)\d{2})/gi,
  ...
  for (const re of patterns) {
    for (const m of text.matchAll(re)) {
      matches.push(m[0].toLowerCase().replace(/\s+/g, ' ').trim());
    }
  }

  // Filter overlapping or redundant matches
  const uniquePositions = matches.filter((m, i) => {
    // If a more specific date (month+year) is already captured, skip the year-only version if it overlaps
    if (/^\d{4}/.test(m)) {
      return !matches.some((other, j) => i !== j && other.includes(m) && other.length > m.length);
    }
    return i === matches.indexOf(m);
  });
