...
import { tokenize, extractNgrams } from '../nlp/tokenizer';
import { detectIndustry, getIndustrySkills } from '../nlp/skills-taxonomy';
...
function extractSkills(
    ...
    const industrySet = new Set(industrySkills.map(s => s.toLowerCase()));
    const fullText = terms.join(' ').toLowerCase();

    // 1. match single terms and multi-word terms from taxonomy
    for (const term of terms) {
        if (industrySet.has(term.toLowerCase()) && term.length >= 2) {
            skills.add(term.toLowerCase());
        }
    }

    for (const phrase of [...bigrams, ...trigrams]) {
        if (industrySet.has(phrase.toLowerCase())) {
            skills.add(phrase.toLowerCase());
        }
    }

    // 2. Also consider skills mentioned within experience/project descriptions
    // We look for patterns like "experience in X", "using X for Y", "X projects"
    const contextPatterns = [
        /(?:experience|proficien(?:cy|t)|skilled|knowledge)\s+(?:in|with|of)\s+([^,.;\n]+)/gi,
        /(?:using|utilizing|leveraging)\s+([^,.;\n]+)/gi,
        /([^,.;\n]+)\s+(?:projects?|development|implementation)/gi
    ];
    ...
    for (const pattern of contextPatterns) {
        ...
        for (const match of matches) {
            ...
            // Check if any industry skills are within this context
    ...
    // 3. catch common skill patterns not in taxonomy
    const skillPatterns = [
        // tech
