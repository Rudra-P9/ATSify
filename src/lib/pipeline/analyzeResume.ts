...
export async function processResumePipeline(file: File, jobDescription?: string): Promise<AnalysisPipelineResult> {
  ...
  const scoringInput: ScoringInput = {
    ...
    experienceBullets: parsedDoc.experience.flatMap(e => e.bullets),
    experienceEntries: parsedDoc.experience,
    educationText: parsedDoc.education.map(e => e.rawText).join('\n\n'),
    educationEntries: parsedDoc.education,
    jobDescription: jobDescription,
    resumeSkills: parsedDoc.skills
  };
